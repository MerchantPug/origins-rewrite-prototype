        ...
        if (pairStream.isPresent()) {
            for (Pair<T, T> pair : pairStream.get().toList()) {
                ...
                if (key.error().isPresent() || key.result().isEmpty()) {
                    Apoli.LOGGER.warn("Failed to decode power id from {}. (Skipping)", pair.getFirst());
                    continue;
                }
                Optional<RegistryEntry.Reference<Power>> power = powerRegistry.getOptional(key.result().get().getFirst());
                if (power.isEmpty()) {
                    Apoli.LOGGER.warn("Failed to get power from id '{}' whilst decoding. (Skipping)", key.result().get().getFirst().getValue());
                    continue;
                }
                PowerType powerType = power.get().value().getType();
                if (!(powerType instanceof DataAttachedPowerType<?> dapt)) {
                    Apoli.LOGGER.warn("Power type '{}' cannot have attached data. (Skipping)", ApoliRegistries.POWER_TYPE.getId(powerType.getSerializer()));
                    continue;
                }
                ...
                if (value.error().isPresent() || value.result().isEmpty()) {
                    Apoli.LOGGER.warn("Failed to decode power data from {}. (Skipping)", pair.getSecond());
                    continue;
                }
    ...
    public <T> DataResult<T> encode(Map<RegistryEntry<Power>, Object> input, DynamicOps<T> ops, T prefix) {
        ...
        for (final Map.Entry<RegistryEntry<Power>, Object> entry : input.entrySet()) {
            if (entry.getKey().getKey().isEmpty()) {
                Apoli.LOGGER.warn("Failed to get power id from a power whilst encoding. (Skipping)");
                continue;
            }
            DataResult<T> encodedKey = REGISTRY_KEY_CODEC.encodeStart(ops, entry.getKey().getKey().get());
            if (encodedKey.error().isPresent()) {
                Apoli.LOGGER.warn("Failed to encode power id '{}' within a power data context. (Skipping) {}", entry.getKey().getKey().get().getValue(), encodedKey.error().get().message());
                continue;
            }
            ...
            if (encodedValue.error().isPresent()) {
                Apoli.LOGGER.warn("Failed to encode power data for power '{}'. (Skipping) {}", entry.getKey().getKey().get().getValue(), encodedKey.error().get().message());
                continue;
            }
